...
frappe.ui.form.on("Job Card", {
  ...
  refresh(frm) {
    ...
    if (frm.doc.qr_code_image && !frm.is_new()) {
      frm.sidebar && frm.sidebar.add_user_action &&
        frm.sidebar.add_user_action(__("Print QR Code")).on("click", function () {
          let w = window.open("");
          w.document.write(
            '<img src="' + frm.doc.qr_code_image + '" style="width:300px;margin:40px auto;display:block;" />' +
            '<p style="text-align:center;font-size:18px;font-weight:bold;">' + frm.doc.name + '</p>'
          );
          w.print();
        });
    }
  ...
  vehicle(frm) {
    ...
    frappe.db.get_value("Vehicle", frm.doc.vehicle,
      ["customer", "make_model", "registration_number"]
    ).then(function (r) {
      if (!r.message) return;
...
frappe.ui.form.on("Job Card Repair Item", {
  qty(frm, cdt, cdn) { computeItemAmount(frm, cdt, cdn); },
  rate(frm, cdt, cdn) { computeItemAmount(frm, cdt, cdn); },
});
