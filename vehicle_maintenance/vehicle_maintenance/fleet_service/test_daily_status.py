...
class DailyStatusTestBase(FrappeTestCase):
	def setUp(self):
		...
		self.addCleanup(setattr, azure_ai, "complete_json", azure_ai.complete_json)
		self.addCleanup(setattr, azure_ai, "is_enabled", azure_ai.is_enabled)
		self.addCleanup(frappe.set_user, "Administrator")

		# Neither provider is reachable unless a test says so. Without this a test
		# machine with credentials in site_config would make real network calls.
		azure_ai.is_enabled = lambda: False
		onyx_client.is_enabled = lambda: False
...
class TestAISummary(DailyStatusTestBase):
	def _with_ai(self, answer: str):
		"""ONYX answering, Azure absent — the prose-and-parse path."""
...
class TestAzureSummary(DailyStatusTestBase):
	"""Azure is preferred because it can be held to the schema; ONYX cannot."""

	ANSWER: ClassVar[dict] = {
		"summary": "PM done, compressor still leaking.",
		"completed": [{"text": "Preventive maintenance finished", "ticket": None, "vehicle": None}],
		"in_progress": [],
		"blockers": [{"text": "Compressor leaking, part needed", "ticket": None, "vehicle": None}],
		"needs_attention": [],
	}

	def _with_azure(self, answer=None, error=None):
		self._configure(use_ai=1)
		azure_ai.is_enabled = lambda: True

		def call(prompt, **kw):
			if error:
				raise error
			return answer if answer is not None else self.ANSWER

		azure_ai.complete_json = call

	def test_azure_answer_becomes_items(self):
		self._fresh_room()
		self._say("finished PM, compressor still leaking")
		self._with_azure()
		doc = frappe.get_doc("VM Daily Status", daily_status.generate_for_user(self.user, "2026-08-13"))
		self.assertEqual(doc.generated_by, "AI")
		self.assertIn("Azure OpenAI", doc.generation_note)
		self.assertEqual({i.bucket for i in doc.items}, {"Completed", "Blocker"})

	def test_azure_is_preferred_over_onyx(self):
		"""Both up: ONYX must not be called at all."""
		self._fresh_room()
		self._say("did the work")
		self._with_azure()

		called = []
		onyx_client.is_enabled = lambda: True
		onyx_client.complete = lambda prompt, **kw: called.append(1) or "{}"

		doc = frappe.get_doc("VM Daily Status", daily_status.generate_for_user(self.user, "2026-08-13"))
		self.assertEqual(called, [], "ONYX was called even though Azure answered")
		self.assertIn("Azure OpenAI", doc.generation_note)

	def test_azure_failure_falls_through_to_onyx(self):
		self._fresh_room()
		self._say("did the work")
		self._with_azure(error=azure_ai.AzureApiError("HTTP 429"))
		onyx_client.is_enabled = lambda: True
		onyx_client.complete = lambda prompt, **kw: json.dumps(self.ANSWER)

		doc = frappe.get_doc("VM Daily Status", daily_status.generate_for_user(self.user, "2026-08-13"))
		self.assertEqual(doc.generated_by, "AI")
		self.assertIn("ONYX", doc.generation_note)

	def test_both_providers_down_still_records_the_day(self):
		self._fresh_room()
		self._say("bus 4102 stuck waiting for a part")
		self._with_azure(error=azure_ai.AzureApiError("HTTP 500"))
		onyx_client.is_enabled = lambda: True

		def boom(prompt, **kw):
			raise onyx_client.OnyxApiError("HTTP 502")

		onyx_client.complete = boom

		doc = frappe.get_doc("VM Daily Status", daily_status.generate_for_user(self.user, "2026-08-13"))
		self.assertEqual(doc.generated_by, "Fallback")
		self.assertIn("Azure", doc.generation_note)
		self.assertIn("ONYX", doc.generation_note)
		self.assertIn("4102", doc.raw_transcript)

	def test_invented_ticket_id_is_dropped_on_the_azure_path_too(self):
		"""The schema constrains the shape, never the truth of an ID."""
		self._fresh_room()
		self._say("worked on the bus")
		self._with_azure(
			answer={
				"summary": "worked",
				"completed": [{"text": "fixed it", "ticket": "TKT-99999", "vehicle": None}],
				"in_progress": [],
				"blockers": [],
				"needs_attention": [],
			}
...
class TestStatusSchema(FrappeTestCase):
	"""Azure's strict mode rejects a schema that breaks any of these rules."""
	...
	def test_every_object_forbids_additional_properties(self):
		for obj in self._walk(daily_status.status_schema()):
			self.assertIs(obj.get("additionalProperties"), False, obj.get("properties", {}).keys())
	...
	def test_nullable_fields_use_a_type_union(self):
		item = daily_status.status_schema()["properties"]["blockers"]["items"]
		self.assertEqual(item["properties"]["ticket"]["type"], ["string", "null"])
		self.assertEqual(item["properties"]["vehicle"]["type"], ["string", "null"])


class TestAzureConfig(FrappeTestCase):
	def setUp(self):
	...
	def test_string_zero_does_not_enable_the_integration(self):
		frappe.conf["azure_ai_enabled"] = "0"
		frappe.conf["azure_ai_api_key"] = "k"
		self.assertFalse(azure_ai.is_enabled())

	def test_url_is_built_from_deployment_and_version(self):
		frappe.conf["azure_ai_endpoint"] = "https://naanri.openai.azure.com/"
		frappe.conf["azure_ai_deployment"] = "gpt-4.1-mini"
		frappe.conf["azure_ai_api_version"] = "2025-03-01-preview"
		self.assertEqual(
			azure_ai._url(azure_ai._settings()),
			"https://naanri.openai.azure.com/openai/deployments/gpt-4.1-mini"
	...
	def test_calling_without_a_key_raises_config_error_rather_than_a_request(self):
		orig = azure_ai._settings
		azure_ai._settings = lambda: {**orig(), "enabled": True, "api_key": None}
		self.addCleanup(setattr, azure_ai, "_settings", orig)
		with self.assertRaises(azure_ai.AzureConfigError):
			azure_ai.complete("hello")
