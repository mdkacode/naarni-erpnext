	...
	<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		...
		<template v-else-if="data.name">
			...
			<div class="bg-white border border-gray-200 rounded-xl p-6">
				...
				<div class="flex items-start justify-between mb-6">
					...
					<div class="flex flex-col items-end gap-2">
						...
						<div
							...
							class="text-xs font-medium text-red-700 bg-red-100 px-2 py-1 rounded"
						>
				...
				<!-- ── Onboarding nudge (Open / Reopened) ── -->
				<div
					v-if="['Open', 'Reopened'].includes(data.workflow_state)"
					class="mb-4 p-3 rounded-lg border border-brand-200 bg-brand-50 flex items-center justify-between gap-3"
				>
					<div class="text-sm text-brand-800">
						<span class="font-semibold">Onboarding pending</span> — assign, fill the checklist,
						and upload Before-Images before starting work.
					</div>
					<router-link
						:to="{ name: 'JobCardOnboarding', params: { jobCard: data.name } }"
						class="shrink-0 px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-600 text-white hover:bg-brand-700"
					>
						Continue →
					</router-link>
				</div>
				...
				<!-- ── Workflow Actions ── -->
				<div
					v-if="(availableActions.length && canEdit) || canForceClose"
				...
				<div
					...
					:class="transitionOk ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'"
				>
					{{ transitionMsg }}
				</div>
				...
				>
					<span class="font-semibold">Customer approval required</span> — parts total exceeds
					₹1,000.
				</div>
				...
				<div class="mb-6">
					<div class="flex items-center justify-between mb-3">
						...
						<div v-if="editing" class="flex gap-2">
							<button @click="cancelEditing" class="text-sm text-gray-500 hover:underline">
								Cancel
							</button>
							<button
								@click="saveEdits"
					...
					<dl v-if="!editing" class="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
						...
						<div v-if="data.force_close_reason" class="col-span-2">
							...
							<dd class="font-medium whitespace-pre-line text-red-700">
							...
							<div>
								...
								>
									...
									<option v-for="u in seOptions" :key="u.user" :value="u.user">
										{{ u.full_name || u.user }}
										<span v-if="u.full_name">({{ u.user }})</span>
									</option>
								</select>
							</div>
							<div>
				...
				<div v-if="showRepairTable" class="mb-6">
					...
					<div v-if="saveRepairsError" class="mt-2 text-sm text-red-600">
						{{ saveRepairsError }}
					</div>
				</div>
				...
				<div v-if="isPmsRepair" class="mb-6">
					...
					<div v-if="saveMaintenanceError" class="mt-2 text-sm text-red-600">
				...
				<div v-if="isBreakdown" class="mb-6">
					...
					/>
					<div
						v-if="saveBreakdownMsg"
						class="mt-2 text-sm"
						:class="saveBreakdownOk ? 'text-green-600' : 'text-red-600'"
					>
						{{ saveBreakdownMsg }}
					</div>
				</div>
				...
				<div v-if="isSoftwareUpdate" class="mb-6">
					...
					<SoftwareUpdatePanel
						...
						:disabled="true"
					/>
					<div v-if="saveSoftwareError" class="mt-2 text-sm text-red-600">
						{{ saveSoftwareError }}
					</div>
				</div>
				...
				<div v-if="showInventory" class="mb-6">
					<InventoryRequestPanel :job-card-name="data.name" :roles="roles" />
				</div>
			...
			<!-- ── Sticky Save / Cancel bar (any section in edit mode) ── -->
			<div
				v-if="anyEditing"
				class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-30 shadow-lg"
			>
				<div class="max-w-4xl mx-auto flex items-center justify-between gap-3">
					<p class="text-xs text-gray-500">
						<span class="font-semibold text-gray-900">Editing</span>
						— click Save in the open section, or Cancel to discard.
					</p>
					<div class="flex gap-2">
						<button
							type="button"
						...
						<button
							type="button"
							@click="saveAnyOpen"
							:disabled="saving || savingRepairs || savingMaintenance || savingSubsystems"
							class="px-4 py-2 text-sm font-semibold rounded-lg bg-brand-600 text-white disabled:opacity-50"
						>
							{{
								saving || savingRepairs || savingMaintenance || savingSubsystems
									? "Saving…"
									: "Save Changes"
							}}
						</button>
...
const canReopen = computed(
	() => data.value.workflow_state === "Closed" && hasAnyRole(roles, ["Customer", "Central Ops"])
);
...
function actionClass(action) {
	...
	if (action === "Verification Failed" || action === "Customer Rejects")
...
// ── Sticky save bar helpers ──
const anyEditing = computed(
	() => editing.value || editingRepairs.value || editingMaintenance.value || editingSubsystems.value
);
function cancelAllEdits() {
	if (editing.value) cancelEditing();
	if (editingRepairs.value) cancelEditRepairs();
	if (editingMaintenance.value) cancelEditMaintenance();
	if (editingSubsystems.value) cancelEditSubsystems();
}
async function saveAnyOpen() {
	// Save whichever section is currently open. Sequenced (not parallel) so
	// server validation errors surface in order.
	if (editing.value) await saveEdits();
	if (editingRepairs.value) await saveRepairs();
	if (editingMaintenance.value) await saveMaintenance();
	if (editingSubsystems.value) await saveSubsystems();
}
</script>
