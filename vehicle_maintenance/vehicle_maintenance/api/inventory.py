...
def submit_inventory_request(
    ...
    if exceeds_threshold:
        ...
        doc.add_comment(
            "Comment",
            _(
                "Inventory request of {0} blocked — exceeds threshold of {1}. "
    ...
    return {
        ...
        "message": _("{0} item(s) added to Job Card {1}.").format(
            len(normalised), doc.name
        ),
    }
...
def release_blocked_inventory(job_card_name: str) -> dict:
    ...
    doc.add_comment("Comment", _("Blocked inventory released — {0} item(s) added.").format(len(pending_items)))
    doc.save()
