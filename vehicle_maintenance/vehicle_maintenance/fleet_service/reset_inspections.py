...
def _runs_for(process_name: str) -> list[str]:
	definitions = frappe.get_all("Process Definition", filters={"process_name": process_name}, pluck="name")
	if not definitions:
		frappe.throw(f"No process is called {process_name!r}. Nothing was touched.")
	return frappe.get_all("Process Run", filters={"process_definition": ["in", definitions]}, pluck="name")
