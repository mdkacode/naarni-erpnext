<template>
  ...
  <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    >
      A Health Card has not yet been generated for this Job Card. It will be
      created automatically once the Service Engineer closes the job.
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="bg-white border border-gray-200 rounded-xl p-3">
          ...
          <div class="font-semibold text-gray-900 mt-1">{{ card.vehicle_number || card.vehicle }}</div>
        </div>
        <div class="bg-white border border-gray-200 rounded-xl p-3">
...
const pdfUrl = computed(() =>
  card.value
    ? `/api/method/frappe.utils.print_format.download_pdf?doctype=Vehicle+Health+Card&name=${encodeURIComponent(card.value.name)}&format=Vehicle+Health+Card+Report&no_letterhead=1`
    : "#"
);
