...
object LocalEvaluator {
	...
	private val NUMERIC_TYPES = setOf(NUMBER, NUMBER_WITH_TOLERANCE, COMPUTED)
	...
	fun evaluate(
		...
		val judged = when {
			...
			else -> if (trimmed.isNullOrEmpty()) {
				null
			} else {
				blank.copy(valueText = trimmed, isAnswered = true, isPass = true)
			}
		} ?: return blank
