...
def create_room(
	...
	Restricted to `Group Admin`, and that is the whole point of the role: this
	used to be open to every operational role that could run a depot, so a room
	got made whenever somebody wanted a side conversation, and the list grew
	faster than anyone could keep track of who was in what.

	`members` is a JSON list of user ids. Returns: {success, data: {room}}.
	"""
	frappe.only_for(["System Manager", "Group Admin"])
...
def add_depot_members(room: str) -> dict:
	...
	Same gate as creating a room: putting twenty people into a channel is the
	other half of the power the `Group Admin` role exists to hold.
