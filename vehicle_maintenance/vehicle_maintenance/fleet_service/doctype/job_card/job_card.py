...
class JobCard(Document):
	...
	def _auto_assign_creator(self) -> None:
		"""If a Service Engineer creates a JC, default-assign it to themselves.

		`assigned_service_engineer` is permlevel:1 — the framework strips it from
		client-side inserts for users who lack permlevel-1 write access. Setting
		it inside the controller runs *after* the perm filter, so the assignment
		sticks. Only fires when the field is blank (preserves explicit picks the
		Depot Manager may have made on web) and only for Service Engineers
		(other roles either get it via the DM picker or shouldn't auto-claim).
		"""
		if self.assigned_service_engineer:
			return
	...
	def _auto_select_check_sheet(self) -> None:
		...
		if odo <= 20_000:
			...
			template_code = "SHEET_A"
		elif odo <= 40_000:
			self.check_sheet = "Sheet B (20,001-40,000 km)"
			template_code = "SHEET_B"
		elif odo <= 80_000:
			self.check_sheet = "Sheet C (40,001-80,000 km)"
			template_code = "SHEET_C"
		else:
			self.check_sheet = "Sheet D (80,001+ km)"
			template_code = "SHEET_D"

		# Resolve structured template — only set if it exists (later sheets are seeded incrementally).
		import frappe

		if not self.check_sheet_template and frappe.db.exists("Check Sheet Template", template_code):
			self.check_sheet_template = template_code
