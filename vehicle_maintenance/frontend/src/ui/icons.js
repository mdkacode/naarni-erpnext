/**
 * NaArNi Design Language — the icon system.
 *
 * One set (Lucide), one stroke weight, one size scale. The geometry is inlined
 * rather than pulled from a runtime package so that there is exactly one way to
 * draw an icon and no way to accidentally mix a filled Material glyph, an emoji
 * and a Feather outline on the same row — which is what the old frontend did,
 * where a phone number was labelled with the emoji 📞.
 *
 * An emoji cannot be tinted, cannot be sized reliably, and renders as a
 * different drawing on every operating system. It is not an icon.
 *
 * Adding an icon: copy the inner markup of the Lucide SVG into the map below,
 * keyed by its Lucide name. Nothing else changes.
 *
 * See DESIGN_LANGUAGE.md §3.9.
 */
export const ICONS = {
	"alert-triangle":
