...
data class LocalRunEntity(
	...
	// ---------------------------------------------------------- where they were
	//
	// The exact check the operator was on when they last put the phone down, and
	// the module it belongs to. Stored rather than derived, because "the first
	// unanswered check" is a good guess and not the same thing: somebody who
	// steps back to re-read check 3 of 26 and then walks away must come back to
	// check 3, not be sent to 11.
	//
	// It is a position, not a record — nothing here is ever sent to the server.
