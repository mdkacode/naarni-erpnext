...
def generate_from_job_card(job_card, *, force: bool = False) -> str | None:
    ...
    existing = frappe.db.get_value(
        "Vehicle Health Card", {"job_card": job_card.name}, "name"
    )
    ...
    for row in category_rows:
        hc.append("category_scores", {
            field: row.get(field) for field in CATEGORY_SCORE_FIELDS
        })
...
def _build_summary(job_card) -> str:
    ...
    parts_count = len([i for i in job_card.get("repair_items", [])
                       if i.get("activity_type") in ("Spare Replacement", "Both")])
    labour_count = len([i for i in job_card.get("repair_items", [])
                        if i.get("activity_type") == "Only Repair"])
    ...
    lines = [
        ...
        _("Summary: {0} parts replaced, {1} labour repairs, {2} maintenance jobs.").format(
            parts_count, labour_count, maint_count,
        ),
    ]
    return "\n".join(lines)
...
def get_for_job_card(job_card_name: str) -> dict:
    ...
    name = frappe.db.get_value(
        "Vehicle Health Card", {"job_card": job_card_name}, "name"
    )
    if not name:
    ...
    return {
        ...
        "data": {
            ...
            "category_scores": [
                {f: row.get(f) for f in CATEGORY_SCORE_FIELDS}
                for row in hc.category_scores
            ],
        },
    }
