...
RECIPIENTS_BREAKDOWN_DECLARED = [
    "Central Ops", "Depot Manager", "Service Engineer", "N. Maintenance Head",
]
RECIPIENTS_REMOTE_RESOLUTION_FAILED = [
    "Service Engineer", "Central Ops", "N. Maintenance Head",
]
...
def notify_job_card_created(doc) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Job Card {0} created for vehicle {1} at depot {2}. "
            "Type: {3}. Priority: {4}."
        ).format(
            doc.name, doc.vehicle_number or "—", doc.depot or "—",
            doc.job_card_type, doc.priority or "Medium",
        ),
        priority="Medium",
    )
...
def notify_customer_approval_outcome(doc, approved: bool) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "The customer has {0} the repair estimate for Job Card {1} "
            "(vehicle {2})."
        ).format(verdict, doc.name, doc.vehicle_number or "—"),
        priority="Medium",
    )
...
def notify_closed_by_technician(doc) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Technician has closed Job Card {0} (vehicle {1}). "
            "Service Engineer verification required."
...
def notify_verified_and_closed(doc) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Job Card {0} (vehicle {1}) has been verified and closed by the Service Engineer."
        ).format(doc.name, doc.vehicle_number or "—"),
        priority="Medium",
    )
...
def notify_reopened(doc) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Job Card {0} (vehicle {1}) was reopened. Please review and re-assign."
        ).format(doc.name, doc.vehicle_number or "—"),
        priority="Medium",
    )
...
def notify_closed_state_edit(doc, reason: str | None = None) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Job Card {0} (vehicle {1}) was modified by {2} after closure. "
            "Reason: {3}"
        ).format(
            doc.name,
            doc.vehicle_number or "—",
...
def _notify_parts_allocated(inv_req) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Parts {0} × {1} have been allocated for Job Card {2} (vehicle {3}). "
            "Ready for issue."
        ).format(
...
def _notify_parts_received(inv_req) -> None:
    ...
    _dispatch(
        ...
        body=_(
            "Parts {0} × {1} acknowledged as received for Job Card {2}. "
            "Work may proceed."
        ).format(
            inv_req.part_name or inv_req.part or "—",
            inv_req.quantity or 1,
...
def _dispatch(job_card, roles, extra_users, subject: str, body: str,
...
def _dispatch_push(user: str, subject: str, body: str,
                   doc_name: str | None, priority: str) -> None:
    """Stub — wire your push provider here (FCM/APNs/OneSignal).
    Triggered only when `notifications_push_enabled` is truthy in site config.
    """
    try:
        # Example placeholder body; replace with a real HTTP call.
        frappe.logger().info(
            f"[push-stub] user={user} doc={doc_name} priority={priority} "
            f"subject={subject!r}"
        )
    except Exception:
        frappe.log_error(
            title=f"Push notification stub failed (user={user})",
            message=frappe.get_traceback(),
        )


def _dispatch_sms(user: str, subject: str, body: str,
                  doc_name: str | None, priority: str) -> None:
    """Stub — wire your SMS provider here (Twilio/MSG91/AWS SNS).
    Triggered only when `notifications_sms_enabled` is truthy in site config.
    The user's `mobile_no` on their User record is the intended destination.
    """
    try:
        mobile = frappe.db.get_value("User", user, "mobile_no")
        frappe.logger().info(
            f"[sms-stub] user={user} mobile={mobile} doc={doc_name} "
            f"priority={priority} subject={subject!r}"
        )
    except Exception:
        frappe.log_error(
            title=f"SMS notification stub failed (user={user})",
            message=frappe.get_traceback(),
        )
...
def _dispatch_email_job(
    ...
    body_html = "".join(paragraphs) or (
        '<p style="margin:0;">See Job Card for details.</p>'
    )
