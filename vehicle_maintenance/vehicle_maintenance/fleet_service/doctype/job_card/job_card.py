...
FORCE_CLOSE_AUTHORITY: dict[str, list[str]] = {
    "Minor": ["Service Engineer", "Depot Manager", "Aftersales Eng",
              "N. Maintenance Head"],
...
TECH_LOCKED_HEADER_FIELDS: tuple[str, ...] = (
    "job_card_type", "repair_subtype", "vehicle", "customer",
    "depot", "priority", "service_contract",
    "assigned_service_engineer", "assigned_technician",
)
...
class JobCard(Document):
    ...
    def autoname(self) -> None:
        ...
        customer_code = (
            frappe.db.get_value("Customer", self.customer, "customer_code") or "CUST"
        )
        location_code = (
            frappe.db.get_value("Depot", self.depot, "location_code") or "DEPOT"
        )
    ...
    def _auto_set_service_type(self) -> None:
        ...
        if self.job_card_type:
            self.service_type = SERVICE_TYPE_BY_JOB_CARD_TYPE.get(
                self.job_card_type, "Other"
            )
    ...
    def _autofill_last_pms_info(self) -> None:
        ...
        if any([self.last_pms_date, self.last_pms_odometer,
                self.last_service_tolerance_level, self.last_serviced_by]):
            return
        ...
        prior = frappe.get_all(
            ...
            fields=[
                ...
                "pms_tolerance_level", "assigned_service_engineer",
                "assigned_technician",
            ],
    ...
    def validate(self) -> None:
        ...
        self._pre_save_workflow_state = (
            None if self.is_new() else self.get_db_value("workflow_state")
        )
    ...
    def on_trash(self) -> None:
        ...
        raise frappe.ValidationError(
            _("Job Cards cannot be deleted, only Voided (use Force Close).")
        )
    ...
    def _broadcast_fleet_risk_if_needed(self, old_state: str | None) -> None:
        ...
        frappe.db.set_value(
            "Job Card", self.name, "fleet_broadcast_sent", 1,
    ...
    def _create_critical_followup_if_needed(self) -> str | None:
        ...
        prior = frappe.db.exists(
            "Job Card", {"source_force_close_job_card": self.name}
        )
        if prior:
            frappe.db.set_value(
                "Job Card", self.name, "followup_job_card", prior,
                update_modified=False,
            )
            return prior
        ...
        frappe.db.set_value(
            "Job Card", self.name, "followup_job_card", followup.name,
            update_modified=False,
        )
    ...
    def _generate_health_card_if_closed(self, old_state: str | None) -> None:
        ...
        if hc_name:
            frappe.db.set_value(
                "Job Card", self.name, "health_card", hc_name,
                update_modified=False,
            )
    ...
    def _enforce_closed_state_lock(self) -> None:
        ...
        if "Administrator" not in user_roles and "Depot Manager" not in user_roles:
            raise frappe.PermissionError(
                _("This Job Card is Closed. Only a Depot Manager can modify it.")
            )
        if not (self.closed_edit_reason or "").strip():
            frappe.throw(
                _("A 'Reason for Closed-State Edit' is mandatory when modifying "
                  "a Closed Job Card.")
            )
    ...
    def _enforce_in_progress_edit_lock(self) -> None:
        ...
        if self.is_new():
            return
        old_state = self.get_db_value("workflow_state")
        locked_states = {"WIP", "Awaiting Parts", "Parts Fitted",
                         "Closure from Technician"}
        if old_state not in locked_states:
            return
        user_roles = set(frappe.get_roles(frappe.session.user))
        # Only enforce when the sole privileged role is Technician.
        if user_roles & {"Service Engineer", "Depot Manager",
                         "Administrator", "Aftersales Eng",
                         "N. Maintenance Head"}:
            return
        ...
        for field in TECH_LOCKED_HEADER_FIELDS:
            ...
            if old_value != self.get(field):
                frappe.throw(
                    _("Technicians cannot edit header field '{0}' while the "
                      "Job Card is in progress. Ask the Service Engineer.").format(field)
                )
    ...
    def _stamp_timestamps(self) -> None:
        ...
        if field and not self.get(field):
            frappe.db.set_value(
                self.doctype, self.name, field, now_datetime(), update_modified=False
            )
    ...
    def _breakdown_onsite_active(self) -> bool:
        ...
        return (
            self.job_card_type == "Breakdown"
            and bool(self.arrived_at_location)
        )
    ...
    def _compute_health_scores(self) -> None:
        ...
        # Group component scores by category (bus_system) first.
        ...
        vehicle_score = round(
            sum(category_percentages.values()) / len(category_percentages), 1
        )

        is_pre_phase = self.workflow_state in (
            "Open", "WIP", "Awaiting Customer Approval",
            "Awaiting Parts", "Parts Fitted",
        )
        is_post_phase = self.workflow_state in (
            "Closure from Technician", "Verification Pending", "Closed",
        )
        ...
        self._sync_category_score_rows(by_category, category_percentages,
                                       is_pre_phase, is_post_phase)
