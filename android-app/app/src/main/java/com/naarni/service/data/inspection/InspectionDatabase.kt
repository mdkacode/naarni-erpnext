...
abstract class InspectionDatabase : RoomDatabase() {
	...
	companion object {
		...
		/**
		 * Adds where the operator was: `lastStage` and `lastStep`.
		 *
		 * A real migration rather than a destructive fallback, and that is not a
		 * style preference. This database holds answers, scans and photographs
		 * that have not reached the server — a shed's worth of an engineer's
		 * afternoon — and `fallbackToDestructiveMigration` would delete all of it
		 * to add two nullable columns.
		 */
		private val MIGRATION_1_2 = object : Migration(1, 2) {
			override fun migrate(db: SupportSQLiteDatabase) {
				db.execSQL("ALTER TABLE runs ADD COLUMN lastStage TEXT")
				db.execSQL("ALTER TABLE runs ADD COLUMN lastStep TEXT")
			}
		}
