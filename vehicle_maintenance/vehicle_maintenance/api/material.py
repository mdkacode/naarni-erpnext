...
def submit_movement(movement: str, remarks: str = "") -> dict:
	...
	# Every item needs a photograph, or an explicit reason it has none.
	#
	# This was advisory and is not any more, because the person who can take the
	# picture is standing in front of the item and the person who needs it is
	# reading the record next month. `no_photo_reason` remains the way past —
	# a sealed crate is a real thing — but it has to be *said*, which is the
	# difference between an exception and a habit. The app enforces the same rule
	# so nobody meets it for the first time at submit; this is what makes it true
	# regardless of which client asked.
	unphotographed = [
		row.item_name or row.item
		for row in doc.items
		if not cint(row.photo_count) and not (row.no_photo_reason or "").strip()
	]
	if unphotographed:
		frappe.throw(
			_("These items still need a photo, or a reason they have none: {0}").format(
				", ".join(unphotographed[:8]) + ("…" if len(unphotographed) > 8 else "")
			)
		)
