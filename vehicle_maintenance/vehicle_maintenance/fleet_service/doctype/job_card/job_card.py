...
class JobCard(Document):
	...
	def _auto_select_check_sheet(self) -> None:
		...
		# Resolve structured PMS Check Sheet — only set if seeded.
		if not self.get("pms_check_sheet") and frappe.db.exists("PMS Check Sheet", template_code):
			self.pms_check_sheet = template_code
	...
	def _validate_workflow_transition(self) -> None:
		...
		# Open / Reopened → WIP gate: enforce the onboarding flow
		# (assigned + checklist 100% + before-images). Same gates the
		# JobCardOnboarding wizard shows; here so Desk and any direct
		# `transition_job_card` call obey them too.
		if old_state in ("Open", "Reopened") and new_state == "WIP":
