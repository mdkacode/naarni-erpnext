...
class DailyStatusSettings(Document):
	def validate(self) -> None:
		...
		if self.use_ai:
			...
			if not (azure_ai.is_enabled() or onyx_client.is_enabled()):
				frappe.msgprint(
					_(
						"No AI provider is enabled, so days will be recorded and emailed in plain "
						"form until one is. Turn on <b>Azure AI Settings</b> (preferred — it can be "
						"held to a fixed JSON shape) or <b>Onyx Settings</b>."
					),
					indicator="blue",
					alert=True,
				)
