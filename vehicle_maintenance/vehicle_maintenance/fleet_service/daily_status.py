...
* **AI is an improvement, never a dependency.** ONYX organises the messages into
  finished / running / stuck. When it is off, unreachable, or replies with
  something that does not parse, the day is still recorded, still counted and
  still emailed — just rendered verbatim. See :func:`_summarise`.
"""
...
_PROMPT = """You are summarising one field technician's own end-of-day messages \
from a vehicle maintenance depot. They wrote these on a phone, often in a mix of \
English and Hindi, often as voice notes that were transcribed roughly.
...
- "blockers" is for work that cannot continue — a missing part, a bus not \
released, waiting on someone. "needs_attention" is for anything a manager should \
know that is not blocking.
- Leave a list empty rather than padding it.
- ticket and vehicle must be one of the reference IDs above, or null."""
...
def _summarise(user: str, status_date: str, rows: list[dict], depot: str | None) -> dict:
	...
	Never raises on an ONYX failure: an unreachable AI degrades the record from
	"organised" to "verbatim", which is a worse report but still a report.
	"""
	...
	if not onyx_client.is_enabled():
		return _fallback(transcript, rows, note="ONYX is not enabled — day rendered verbatim.")
	...
	prompt = _PROMPT.format(
		...
		date=status_date,
		transcript=transcript_for(sent_rows),
		tickets=", ".join(tickets) or "none",
		vehicles=", ".join(vehicles) or "none",
	)

	try:
		answer = onyx_client.complete(
			prompt,
			description=f"daily-status/{depot or 'no-depot'}/{status_date}/{user}",
		)
	except (onyx_client.OnyxConfigError, onyx_client.OnyxApiError) as exc:
		frappe.logger("naarni").warning(f"daily status: ONYX unavailable for {user}: {exc}")
		return _fallback(transcript, rows, note=f"ONYX unavailable: {exc}"[:400])
	...
	if not parsed:
		return _fallback(
			transcript,
			rows,
			note="ONYX replied with something that was not JSON.",
			raw=answer,
		)
	...
	if not items and not summary:
		return _fallback(transcript, rows, note="ONYX returned an empty summary.", raw=answer)
