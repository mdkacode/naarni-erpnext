				...
				<div class="space-y-5">
					...
					<div>
						...
						<div class="relative">
							...
							<!-- Dropdown results -->
							<div
								v-if="showVehicleDropdown && vehicleResults.length"
									...
									<div class="flex items-center justify-between">
										<div>
											<span class="font-bold text-gray-900">{{
												formatVehicle(v.registration_number)
											}}</span>
						...
						>
							<div>
								<span class="font-bold text-green-800 text-lg tracking-wide">{{
									formatVehicle(data.vehicle_number)
								}}</span>
								<span class="text-sm text-green-700 ml-2">{{ data.vehicle_make_model }}</span>
							</div>
							<button
								type="button"
								@click="clearVehicle(updateField)"
					...
					<div v-if="data.customer_name" class="p-3 bg-gray-50 rounded-xl text-sm space-y-1">
						...
						<div v-if="data.auto_check_sheet" class="flex justify-between">
							<span class="text-gray-500">Check Sheet</span
							><span class="font-medium">{{ data.auto_check_sheet }}</span>
						</div>
					</div>
				</div>
			</template>
			...
			<template #step-details="{ data, updateField }">
				<div class="space-y-5">
					...
					<div>
						...
						<div v-if="data.depot" class="mt-2 text-sm text-green-700 font-medium">
							Selected: {{ data.depot }}
						</div>
					</div>
					...
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1"
							>Driver / Customer Complaints</label
						>
						<textarea
							:value="data.complaint_description"
							@input="updateField('complaint_description', $event.target.value)"
							rows="3"
							placeholder="What did the driver or customer report? List all issues..."
							class="input-field"
						/>
					</div>
					...
					<div>
						<label class="block text-sm font-medium text-gray-700 mb-1"
							>Your Observations (SE)</label
						>
						<textarea
							:value="data.se_observations"
							@input="updateField('se_observations', $event.target.value)"
							rows="3"
							placeholder="Issues you found during initial inspection..."
							class="input-field"
						/>
					</div>
				</div>
				...
				<div class="space-y-4">
					...
					<div class="bg-gray-50 rounded-xl p-5 space-y-3 text-sm">
						<div class="flex justify-between">
							...
							><span class="font-bold text-gray-900">{{ data.job_card_type }}</span>
						</div>
						<div class="flex justify-between">
						...
						<div class="flex justify-between">
							...
							><span class="font-medium">{{ data.vehicle_make_model }}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-500">Odometer</span
							><span class="font-medium"
								>{{ Number(data.odometer_reading || 0).toLocaleString("en-IN") }} km</span
							>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-500">Customer</span
						...
						<div class="flex justify-between">
							...
							><span class="font-medium">{{ data.depot }}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-500">Priority</span>
							<span
								class="font-semibold"
								:class="data.priority === 'Urgent' ? 'text-red-600' : ''"
								>{{ data.priority }}</span
							>
						</div>
					</div>
...
const steps = [
	...
	},
	{
		...
		validate: (data) => {
			...
			if (!data.complaint_description?.trim())
				errors.push("Please record the driver/customer complaints.");
			return errors;
		},
	},
	{
...
async function handleSubmit(data) {
	...
	try {
		...
		// Drop straight into the onboarding wizard so the user can't skip
		// assignment / checklist / before-images.
		router.push(`/service-portal/job-card/${result.name}/onboarding`);
	} catch (e) {
