...
def get_depot_manager_dashboard(depot: str = "") -> dict:
    ...
    states = frappe.db.sql("""
        ...
        ORDER BY count DESC
    """.format(where=f"WHERE depot = %(depot)s" if depot else ""),
        {"depot": depot} if depot else {}, as_dict=True)

    # SLA breaches
    breached = frappe.db.count("Job Card", filters={**filters, "sla_breached": 1, "workflow_state": ["!=", "Closed"]})
    ...
    priorities = frappe.db.sql("""
        ...
        {depot_filter}
        GROUP BY priority
    """.format(depot_filter=f"AND depot = %(depot)s" if depot else ""),
        {"depot": depot} if depot else {}, as_dict=True)

    # Recent job cards
    recent = frappe.get_list("Job Card", filters={**filters, "workflow_state": ["!=", "Closed"]},
        fields=["name", "vehicle_number", "vehicle_make_model", "job_card_type", "priority", "workflow_state", "sla_breached", "depot", "customer_name", "modified"],
    ...
    fleet = frappe.db.sql("""
        ...
        ORDER BY count DESC
    """.format(join=f"INNER JOIN tabDepot d ON 1=1" if not depot else ""),
        as_dict=True)
...
def get_customer_dashboard() -> dict:
    ...
    # Their vehicles
    vehicles = frappe.get_list("Vehicle", filters={"customer": customer},
        fields=["name", "registration_number", "make_model", "fuel_type", "color", "year_of_manufacture"],
        order_by="registration_number asc", limit_page_length=100)

    # Active job cards
    active_cards = frappe.get_list("Job Card",
        filters={"customer": customer, "workflow_state": ["!=", "Closed"]},
        fields=["name", "vehicle_number", "vehicle_make_model", "job_card_type", "priority",
                "workflow_state", "sla_breached", "estimated_cost", "depot", "modified"],
        order_by="modified desc", limit_page_length=50)

    # Closed cards (history) with health scores
    history = frappe.get_list("Job Card",
        filters={"customer": customer, "workflow_state": "Closed"},
        fields=["name", "vehicle_number", "job_card_type", "pre_pms_score", "post_pms_score",
                "score_improvement", "estimated_cost", "actual_cost", "closed_at", "depot"],
        order_by="closed_at desc", limit_page_length=50)
    ...
    """, {"customer": customer}, as_dict=True)[0]
